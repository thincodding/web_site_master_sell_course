        ...
        <div class="flex justify-between my-5">
            ...
            <div class="flex gap-3">
                <button @click="onMountedCurrentComponents('AddStudentRegisterModal')"
                    class="bg-background px-5 py-2.5 text-white flex items-center gap-1 hover:bg-background/90">
...
export default {
    ...
    setup() {
        ...
        // Computed property for filtered student details
        const filteredStudentDetails = computed(() => {
            if (!searchText.value.trim()) {
                return studentDetails.value;
            }

            const lowerSearch = searchText.value.toLowerCase();

            // Deep clone to avoid mutating original data
            return studentDetails.value.map(cate => {
                const filteredProducts = cate.product.map(pro => {
                    const filteredProductDetails = pro.productDetail.map(detail => {
                        const filteredStudents = detail.student.filter(stu =>
                            stu.studentName.toLowerCase().includes(lowerSearch) ||
